...
bool TemplatedVocabulary<TDescriptor, F>::loadFromBinaryFile(const std::string &filename) {
	...
	m_nodes[0].id = 0;
	char* buff = new char[size_node];
	int nid = 1;
	while (!f.eof()) {
		f.read(buff, size_node);
		m_nodes[nid].id = nid;
		// FIXME
		const int* ptr = (int*)buff;
		m_nodes[nid].parent = *ptr;
		//m_nodes[nid].parent = *(const int*)buff;
		m_nodes[m_nodes[nid].parent].children.push_back(nid);
		m_nodes[nid].descriptor = cv::Mat(1, F::L, CV_8U);
		memcpy(m_nodes[nid].descriptor.data, buff + 4, F::L);
		m_nodes[nid].weight = *(float*)(buff + 4 + F::L);
		if (buff[8 + F::L]) { // is leaf
			int wid = m_words.size();
			m_words.resize(wid + 1);
			m_nodes[nid].word_id = wid;
			m_words[wid] = &m_nodes[nid];
